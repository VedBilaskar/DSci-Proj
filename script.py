...
#from pdfminer.high_level import extract_embedded_files
...
for j in os.listdir(path):
        ...
        # Get the list of embedded file names
        embedcount= doc.embfile_count()

        #embedded_files = doc.list_embeddedfiles()

        # Get the count of embedded files
        #embedcount = len(embedded_files)
...
var =  str(r"tr '\n' ','")
command = ""
header1 = ['Header','contains','obj','endobj','stream','endstrean','xref','trailer','startxref','pageno' ,'encrypt','ObjStm','JS','Javascript','AA','OpenAction','Acroform','JBIG2Decode','RichMedia','launch','EmbeddedFile','XFA','Colors']
with open(os.path.relpath("pdfid/output.csv"),'w',encoding='UTF8') as output:
        output.write(','.join(header1))
        os.chdir('pdfid')
        t0 = time.time()
        for j in os.listdir(path):
         f = path + "/" + j
         out = subprocess.getoutput("python3 pdfid.py "+f+" | awk '{print $2}'| tail -n +1 | "+var+"")
         output.write("\n" + out)
         d = time.time() - t0
